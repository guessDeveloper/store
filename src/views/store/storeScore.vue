<template>
  <div class="menager-box">
    ...
    <div class="tab-content" v-show="tab==1">
      <div class="btn-box">
        <span class="score">我的剩余积分：<span class="">{{Integral.SurplusIntegral}}</span>累计积分：<span>{{Integral.RechargeIntegral}}</span> 已赠送积分：<span>{{Integral.SendIntegral}}</span></span>
        <router-link class="chong-btn" tag="button" to="/storeMoney">立即获得</router-link>
      </div>
      <div class="score-box">
      ...
      <div class="page-box">
        <el-pagination @current-change="getChongList" :current-page.sync="chongIndex" :page-size="chongPage" :hide-on-single-page="chongTotal == 0" layout="prev, pager, next, jumper" :total="chongTotal">
        </el-pagination>
      </div>
      <div class="page-box small">
        <el-pagination small @current-change="getChongList" :current-page.sync="chongIndex" :page-size="chongPage" :hide-on-single-page="chongTotal == 0" layout="prev, pager, next" :total="chongTotal">
        </el-pagination>
      </div>
    </div>
    <div class="tab-content" v-show="tab==2">
      <div class="table-box jiangli">
        <el-table :data="jiangList" header-row-style="font-size:12px;color:#999;" row-class-name="table-line" width="930">
          ...
          <el-table-column property="UserName" label="消费者身份号" width="180" align="center">
          </el-table-column>
          <el-table-column property="Amount" label="消费金额(元)" width="180" align="center">
          </el-table-column>
          <el-table-column property="RebateRatio" label="奖励比例(%)" width="180" align="center">
          </el-table-column>
      ...
      <div class="table-small-box">
        <div class="list-item" v-for="(item,index) in jiangList" :key="index">
          ...
          <div class="list-item-p">
            <p>消费者身份号:<span>{{item.UserName}}</span></p>
          ...
          <div class="list-item-b">
            <p>奖励比例：<span>{{item.RebateRatio}}%</span></p>
          </div>
        </div>
      </div>
      <div class="page-box">
        <el-pagination @current-change="getJiangList" :current-page.sync="jinagIndex" :page-size="jiangPagesize" :hide-on-single-page="jiangTotal == 0" layout="prev, pager, next, jumper" :total="jiangTotal">
        </el-pagination>
      </div>
      <div class="page-box small">
        <el-pagination small @current-change="getJiangList" :current-page.sync="jinagIndex" :page-size="jiangPagesize" :hide-on-single-page="jiangTotal == 0" layout="prev, pager, next" :total="jiangTotal">
        </el-pagination>
      </div>
    </div>
  </div>
