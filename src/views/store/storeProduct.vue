  ...
  <div class="storeProduct-box">
    ...
    <div class="order-content" v-show="tab == '1'" v-loading="loading">
      <div class="choose-box">
        <div class="date-box">
          <el-date-picker v-model="dataValue" type="daterange" value-format="yyyy-MM-dd" range-separator="-" :editable="false" :clearable="false" start-placeholder="开始日期" end-placeholder="结束日期" @change="getList">
          </el-date-picker>
        </div>
        <div class="date-small-box">
          <div class="small-date-box">
            <el-date-picker v-model="dataValue[0]" type="date" :editable="false" :clearable="false" value-format="yyyy-MM-dd" placeholder="开始日期" @change="getList">
            </el-date-picker>
          </div>
          <div class="date-middle"> -</div>
          <div class="small-date-box">
            <el-date-picker v-model="dataValue[1]" type="date" :editable="false" :clearable="false" value-format="yyyy-MM-dd" placeholder="结束日期" @change="getList">
            </el-date-picker>
          </div>
        </div>
        <div class="status-select status-select-wrap">
          ...
          <span class="select">
            <el-select v-model="status" placeholder="请选择" @change="getList">
              <el-option v-for="item in statusOption" :key="item.status" :label="item.label" :value="item.status">
              </el-option>
            </el-select>
          </span>
        </div>
...
export default {
  ...
  mounted() {
    ...
    this.dataValue = [this.$util.getTwoWeek(), this.$util.getNowDate()]
    this.getClass();
    this.getList();
  },
  components: {
