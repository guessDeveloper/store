<template>
  <div class="safe-box">
    ...
    <div class="safe-content" v-show="!isChangingPhone">
      <div class="user">您的当前昵称:<span>{{storeInfo.Name}}</span></div>
      <div class="item" v-show="storeInfo.IsPhoneExist != 1">
      ...
      <div class="item" v-show="storeInfo.IsPhoneExist != 1">
        ...
        <div class="des">
          <div class="name">手机号: {{phoneLimit(storeInfo.phone)}}</div>
    ...
    <el-dialog title="修改密码" :visible.sync="changePas" width="520px" class="small">
      <div class="change-box">
        ...
        <div class="input-line">
          <label for="">短信验证码：</label>
          <div class="input-box"><input type="text" placeholder="输入短信验证码" v-model.trim="changeCode"><button class="btn" @click="getCode">{{codeBtn}}</button></div>
        </div>
        <div class="input-line">
          <label for="">输入原密码：</label>
          <div class="input-box"><input type="password" placeholder="请输入原密码" max-length="20" v-model.trim="oldPas"></div>
        </div>
        <div class="input-line">
          <label for="">输入新密码：</label>
          <div class="input-box"><input type="password" placeholder="请输入新密码" max-length="20" v-model.trim="newPas"></div>
        </div>
        <div class="input-tip"> 请输入最小为8位至少包含1个大写字母，1个小写字母和1个数字的密码,可使用标点符号并且不能连续输入相同大小写字母数字3次或3次以上</div>
        <div class="input-line">
          <label for="">再次输入：</label>
          <div class="input-box"><input type="password" placeholder="请再次输入登录密码" max-length="20" v-model.trim="reNewpas"></div>
        </div>
        <div class="btn-box">
...
export default {
  ...
  mounted() {
    let userInfo = sessionStorage.getItem('storeUserInfo') ? JSON.parse(sessionStorage.getItem('storeUserInfo')) : {};
  ...
  methods: {
    ...
    getSetCode() {
      ...
      if (this.$util.testPhone.test(this.setPhoneNum)) {
        this.lock = true;
        this.$http.storePost(this.$api.SendMerchantThird, {
          UserPhoneNO: this.setPhoneNum,
        }).then(res => {
          if (res.data.Code == 1) {
            this.setCode()
          } else {
    ...
    setSafePhone() {
      if (this.$util.testPhone.test(this.setPhoneNum)) {
        if (this.setPhoneCode != '') {
          if (this.setPhonePass != '' && this.setPhonePass.length >= 8) {
            if (this.setPhonePass != this.repeatPhonePass) {
              this.$message.error('两次输入密码不一致')
            } else {
              this.$http.storePost(this.$api.ImproveMerchantinformation, {
                UserPhoneNO: this.setPhoneNum,
                ValidCode: this.setPhoneCode,
    ...
    getCode() {
      if (this.lock) return false
      this.lock = true;
      this.$http.storePost(this.$api.SendChangePwdVerifyCode).then(res => {
        if (res.data.Code == 1) {
          this.setCode()
        } else {
          this.lock = false
    ...
    //重置密码
    resetPa() {
      this.$http.storePost(this.$api.ChangePwd, { VerifyCode: this.changeCode, OldPwd: this.oldPas, NewPwd: this.newPas }).then(res => {
        if (res.data.Code == 1) {
    ...
    changePhoneCode() {
      if (this.lock) return false
      this.lock = true;
      this.$http.storePost(this.$api.SendChangeTelPhoneVerifyCode).then(res => {
        if (res.data.Code == 1) {
          this.setCode()
        } else {
          this.lock = false;
    ...
    goStepTwo() {
      if (this.oldPhoneCode == '') {
        ...
        this.$http.storePost(this.$api.VerifyChangeTelPhone, { VerifyCode: this.oldPhoneCode }).then(res => {
          if (res.data.Code == 1) {
            this.step = 2
          } else {
            this.$message.error(res.data.Msg)
    ...
    getNewPhoneCode() {
      if (!this.$util.testPhone.test(this.newPhone)) {
        ...
        this.$http.storePost(this.$api.SendChangeTelPhoneNewPhoneVerifyCode, {
    ...
    goStepThree() {
      if (!this.$util.testPhone.test(this.newPhone)) {
        ...
        this.$http.storePost(this.$api.ChangeMerchantTelPhone, {
          NewPhone: this.newPhone,
