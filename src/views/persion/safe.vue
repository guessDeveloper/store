<template>
  <div class="safe-box">
    ...
    <div class="safe-content" v-show="!isChangingPhone">
      ...
      <div class="item" v-show="userInfo.IsPhoneExist != 1">
  ...
  methods: {
    ...
    getSetCode() {
      ...
      if (this.$util.testPhone.test(this.setPhoneNum)) {
        this.lock = true;
        this.$http.limitPost(this.$api.SendUserThird, {
        ...
        if (this.setPhoneCode != '') {
          if (this.setPhonePass != '' && this.setPhonePass.length >= 8) {
            if (this.setPhonePass != this.repeatPhonePass) {
              ...
              this.$http.limitPost(this.$api.ImproveUserinformation, {
                UserPhoneNO: this.setPhoneNum,
    ...
    changePaswrod() {
      if (this.changeCode == '') {
        ...
        return false
      } else if (this.newPas.length < 8) {
        this.$message.error('请输入8至20位登录密码')
        return false
