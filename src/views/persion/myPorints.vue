  ...
  <div class="myPorint-box">
    ...
    <el-dialog title="获取记录" :visible.sync="dialogFormVisible" width="520px" class="small">
      <el-table :data="gridData" style="width:480px;" header-row-style="'font-size:12px;color:#999;'" row-class-name="table-line" class="small-table">
        <el-table-column property="OrderNumber" label="订单号" width="124" align="center" fontSize="12px" cell-style="font-size:12px;"></el-table-column>
        <el-table-column property="ConsumptionTime" label="时间" width="124" align="center"></el-table-column>
        <el-table-column property="PlatformType" label="获取方式" width="82" align="center"></el-table-column>
        <el-table-column property="FromName" label="商家名称" width="82" align="center"></el-table-column>
        ...
        <div class="small-box" v-for="(item,index) in gridData" :key="index">
          <div>
            ...
            <div>
              ...
              <div><span class="name">获取方式:</span>{{item.PlatformType}}</div>
              <div><span class="name">商家名称:</span>{{item.FromName}}</div>
      ...
      <div class="page-box">
        <el-pagination :page-size="onePageSize" :current-page.sync="onePageIndex" layout="prev, pager, next" @current-change="getSorceList" :total="gridDatatotal">
        </el-pagination>

      </div>
      <div class="page-box small">
        <el-pagination small :page-size="onePageSize" :current-page.sync="onePageIndex" layout="prev, pager, next" @current-change="getSorceList" :total="gridDatatotal">
        </el-pagination>
      </div>
    </el-dialog>
    <el-dialog title="兑换记录" :visible.sync="duihuan" width="520px" class="small" @opened="duihuanSuccess = false">
      ...
      <div class="page-box">
        <el-pagination @current-change="getXiaofei" :page-size="twoPageSize" :current-page.sync="twoPageIndex" layout="prev, pager, next" :total="changeDataTotal">
        </el-pagination>
      </div>
      <div class="page-box small">
        <el-pagination small @current-change="getXiaofei" :page-size="twoPageSize" :current-page.sync="twoPageIndex" layout="prev, pager, next" :total="changeDataTotal">
        </el-pagination>
      </div>
    </el-dialog>
  ...
  methods: {
    ...
    duhuan() {
      if (this.UserIntegral == '' || this.UserIntegral == '0') {
        this.$message.error('您目前还未赚取积分，请继续努力哟，加油！')
        return false
      }
